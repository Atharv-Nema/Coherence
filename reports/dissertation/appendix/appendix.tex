...
\begin{itemize}
    ...
    \item $\Gamma_{callable}$: a metadata structure of the current callable (function, constructor, or behaviour) being type-checked
\end{itemize}
...
The \textsf{UserMutex} structure contains the following fields:
\begin{itemize}
  \item \textbf{\textsf{holding\_instance}}: An optional that stores the ID of the actor instance holding the \textsf{UserMutex} (if any).
  \item \textbf{\textsf{num\_lock\_called}}: The net lock depth (difference between the number of \textsf{Lock} and \textsf{Unlock} calls).
  \item \textbf{\textsf{wait\_queue}}: A FIFO queue storing the IDs of waiting actor instances.
  \item \textbf{\textsf{coord\_lock}}: OS-level mutex for safe concurrent access to internal members of \textsf{UserMutex}.
\end{itemize} 
\section{Locking}
\begin{algorithmic}[1]
\Procedure{Lock}{$locker\_id$}
    \State \textbf{acquire} \textsf{coord\_lock}
    
    \If{$\textsf{holding\_instance} = \textsf{null}$}
        \State $\textsf{holding\_instance} \gets \textsf{locker\_id}$
        \State $\textsf{num\_lock\_called} \gets 1$
        \State \textbf{release} \textsf{coord\_lock}
        \State \Return \textbf{true}
    \ElsIf{$\textsf{holding\_instance} = \textsf{locker\_id}$}
        \State $\textsf{num\_lock\_called} \gets \textsf{num\_lock\_called} + 1$
        \State \textbf{release} \textsf{coord\_lock}
        \State \Return \textbf{true}
    \Else
        \State Set \textsf{locker\_id}'s state to \textbf{WAITING} and add it to \textsf{wait\_queue}
        \State \textbf{release} \textsf{coord\_lock}
...
\section{Unlocking}
\begin{algorithmic}[1]
\Procedure{Unlock}{\textsf{runtime}}
    \State \textbf{acquire} \textsf{coord\_lock}
    \State $\textsf{num\_lock\_called} \gets \textsf{num\_lock\_called} - 1$
    
    \If{$\textsf{num\_lock\_called} > 0$}
        \State \textbf{release} \textsf{coord\_lock}
        \State \Return
    \EndIf

    \If{\textsf{wait\_queue} is empty}
        \State $\textsf{holding\_instance} \gets \textsf{null}$
    \Else
        \State $\textsf{target} \gets \textsf{wait\_queue.pop\_front()}$
        \State $\textsf{holding\_instance} \gets \textsf{target}$
        \State $\textsf{num\_lock\_called} \gets 1$
        
        \State Set \textsf{target}'s state to \textbf{RUNNABLE} and add it to the \textit{schedule\_queue}
    \EndIf
    \State \textbf{release} \textsf{coord\_lock}
\EndProcedure
\end{algorithmic}
...
% for K1 > K2 is leads to unsoundness when a pointer of type 
% (T K2) K1 is present as a parameter of a behaviour. 
