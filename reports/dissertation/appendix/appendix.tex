...
\chapter{\textit{UserMutex implementation}}
\section{Locking}
\begin{algorithmic}[1]
...
% ptr_covariant(K, K1, K2): succeeds if the type (T K) K2 is 
% covariant to (T K) K1
...
% K2 is covariant to K1, but the pointer type (T K) K2 is not 
% covariant to (T K) K1.
...
% K2 is invariant to K1, but the pointer type (T K) K2 is not 
% invariant to (T K) K1.
...
% iso > K is not compatible with iso_cap > K as defined in
% section 4.1.3.
