...
\section{Full coherence}
This is the entire version of the language.
\subsection{Formal syntax}

The full language extends the cut-down version with additional capabilities, viewpoint-adapted types, nested pointer types, a \texttt{nullptr} value, and a \texttt{consume} expression for transferring ownership.
...
\kappa & ::= & \mathtt{tag} \mid \mathtt{ref} \mid \mathtt{val} \mid \mathtt{iso} \mid \mathtt{iso\_cap} \mid \mathtt{locked}\langle L \rangle & \text{(reference capabilities)} \\[0.5em]
T_N & ::= & T \mid \mathtt{struct}\ \{\ \overline{f : T;}\ \} & \text{(nameable type)}
\end{array}
\]

\subsubsection{Top-level items}
The top-level structure is identical to the cut-down version.


\subsubsection{Statements}

Identical to the cut-down version.

\subsubsection{Expressions}

The full language adds two new expression forms: a \texttt{nullptr} literal and a \texttt{consume} expression. The \texttt{consume} expression transfers ownership of an \texttt{iso}-capability variable, rendering the original binding unusable afterwards.

\[
\begin{array}{lcll}
e & ::= & \cdots & \text{(same as in the cut-down version)} \\
  & \mid & \mathtt{nullptr} & \text{(null pointer)} \\
  & \mid & \mathtt{new}\ \kappa\ [e]\ T\ (e) & \text{(allocation, extended)} \\
  & \mid & \mathtt{consume}(x) & \text{(ownership transfer)}
\end{array}
\]


Compared to the cut-down version, allocation now takes a full type $T$ rather than a base type $B$ as the element type, enabling allocation of pointer arrays.


\subsection{Type checking}

All expression typing rules from the cut-down version carry over unchanged, except for the following modified and new rules.
...
\begin{prooftree}
    ...
    \AxiomC{$\text{apply\_viewpoint}(\kappa, T) = T_{view}$}
    \AxiomC{$\Gamma \tack \text{type\_assignable}(T_{view}, T')$}
    \RightLabel{(Alloc)}
    \QuaternaryInfC{$\Gamma \tack \text{new } \kappa[e_s] T(e_d) : \text{unalias}(T\ \kappa)$}
\end{prooftree}
...
\begin{prooftree}
    \AxiomC{$\Gamma_V(x) = T$}
    \RightLabel{(Consume)}
    \UnaryInfC{$\Gamma \tack \mathtt{consume}(x) : \text{unalias}(T)$}
\end{prooftree}

\vspace{1em}

\begin{prooftree}
    \AxiomC{$\Gamma \tack e :  \kappa \triangleright \mathtt{Named}(N) $}
    \AxiomC{$\text{get\_struct\_type}(N, \Gamma) = \text{struct } \{ \overline{f:T_f} \}$}
    \AxiomC{$f_i \in \overline{f}$}
    \RightLabel{(Field-Access)}
    \TrinaryInfC{$\Gamma \tack e.f_i : \text{apply\_viewpoint}(\kappa, T_{f_i})$}
\end{prooftree}

\begin{prooftree}
    \AxiomC{$\Gamma \tack e_p : B\ \kappa_p$}
    \AxiomC{$\Gamma \tack e_i : \text{Int}$}
    \AxiomC{$\Gamma \tack \text{ref\_readable}(\kappa_p)$}
    \RightLabel{(Ptr-Access)}
    \TrinaryInfC{$\Gamma \tack e_p[e_i] : \text{apply\_viewpoint}(\kappa_p, B)$}
\end{prooftree}
