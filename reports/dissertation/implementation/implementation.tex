...
\begin{itemize}
    \item We begin by introducing the formal syntax and type-checking rules of \textit{Coherence} (\Cref{sec:theory}).
    \item We then describe the compiler frontend (\Cref{sec:frontend}) and the AST validation stage where the typing rules are implemented and lock-inference is performed (\Cref{sec:ast-validation}).
...
\end{figure}
  Edges A -> B in the graph mean that $B <: A$. So, \texttt{iso\textasciicircum} is assignable to all other capabilities, and \texttt{iso} is not assignable to anything (including itself) to maintain the single-alias invariant. A reference with a locked capability is assignable to another reference with a locked capability if they are protected by the same lock.
...
\begin{itemize}
  ...
  \item
    ...
    \DisplayProof
    \vspace{1.5em}\\
    The \texttt{sendable} predicate is the additional check that was being talked about previously. This checks that none of the behaviour arguments is a reference protected by the \texttt{ref} capability. The \texttt{valid\_var\_usage} predicate checks that unaliased variables are used safely.
\end{itemize}
...
The core type checker is responsible for implementing the typing judgements for expressions and statements as described in \Cref{sec:theory}. It is fairly straightforward to implement these rules efficiently. 
...
\end{minted}
To create an instance of \texttt{list ref}, we need to create an instance of the structs field, which is also of type \texttt{list ref}. There is no end to this recursion! There are a few approaches to solve this problem:
\begin{itemize}
