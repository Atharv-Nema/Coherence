...
\end{notebox}
Iterating through the reverse topological ordering of the call graph ensures that when the time comes to compute the lock-set of a callable, all the dependencies have already have their lock-set computed. Hence, only a single pass is needed to compute the lock-set!
\end{document}