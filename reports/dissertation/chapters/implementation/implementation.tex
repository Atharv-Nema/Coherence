...
% Scheduling entities:
%     N worker threads responsible for ...
%     Scheduler queue for ...
% Actor Instance information
%     A map from instance of actorinstance ids
%     Actor instances themselves, and a description of their fields.
% Mutex's
%     MutexMap and the information about the mutex or something
% Runtime traps
%     Explanation of the runtime traps
% Integrating them together
%     Talk a bit about the entire loop of things happening and how it interacts with 
%     the generated code.
% Software engineering concepts used to prevent dataraces and deadlocks
\subsubsection{Scheduling entities}
The runtime maintains a global schedule queue that stores information about runnable actor-instances. The runtime spawns $N$ OS-threads at the start of the program  responsible for scheduling and executing actor work. \textit{Coherence} follows $M:N$ scheduling, and hence the number of worker threads does not grow with the number of actor-instances. Each scheduler thread repeatedly pulls runnable actor-instances from the global schedule queue and executes their pending behaviour calls. %CR: Perhaps talk about termination of the program here (or perhaps this could be talked about in integration)
