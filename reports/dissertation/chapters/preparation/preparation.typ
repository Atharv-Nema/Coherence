...
- The runtime maintains a global _ready queue_ of actor-instances prepared for execution. Each actor-instance has a mailbox, which contain all of its pending behaviour calls. A fixed pool of worker threads pick actors from the ready queue and execute their behaviours.
