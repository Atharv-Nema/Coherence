...
Developing a language that guarantees deadlock-free and data-race free execution while retaining expressive power requires a synthesis of type theory and low-level systems programming. This chapter goes through the foundational concepts required to motivate and develop _Coherence_
...
stroke: (left: 2pt + gray.lighten(50%))
  ...
  *Note on Synchronous Functions:* Pony also supports synchronous functions (`fun`) that can be called outside the actor-instance. In _Coherence_ however, functions are strictly internal and can only be invoked by the actor's own behaviours. They serve exclusively as private helpers to organize logic, ensuring that all external interaction remains asynchronous.
]
...
A reference capability is a part of the type of every reference. In the example above, an `Account` instance with the capability `ref` has been created. This restricts how `Account` can be used.
...
The following table defines the result of the adaptation operator $O vpt F$ in Pony, where $O$ represents the capability of the originating reference and $F$ represents the capability of the field being accessed.
...
#block(
  ...
  *Sendable callbacks:*
  A _sendable_ callback is a first-class function that can be dispatched to other actors and executed within their respective execution contexts without risk of data-races. Since _Coherence_ does not implement first-class functions, the specific type-checking rules governing sendable closures will not be discussed. The provided code serves only as a conceptual illustration of the manager pattern.
]
...
=== Hot-Potato Model
To solve the atomicity problem, developers can use the *hot-potato* model. Here, the manager transfers exclusive ownership of the object to the requester using the `iso` reference capability. The ownership is returned when the requester is done with the task.
...
This code can be made more robust by ensuring that the `take` operation never fails. If the `IntManager2` is currently empty (because another actor is holding the potato), it stores the incoming callback in a FIFO queue. When the potato is eventually returned via `give`, the manager automatically dispatches it to the next callback in the queue.
...
// Elaborate the issue and expand on the example
...
The standard solution to shared-memory is to protect the shared structure with a lock, making sure that only one concurrent unit is accessing it at the same time. What if we can somehow incorporate this in the reference capability model? The Kappa type system by Elias Castegren does this using the `locked` reference capability. An object with this capability comes with an implicit lock. Whenever a method of the object is called, the lock associated with the object is taken, which is released after the method execution completes. This object can be shared between actors, and hence allows for easy sharing of mutable state.
...
_Coherence_ draws inspiration from Gudka work on automatic lock-inference for Java. Gudka's system _Lockguard_ translates Java code containing atomic sections to an equivalent program that implements these atomic sections using locks. _Lockguard_ performs a lock-set inference on an atomic section to determine the locks needed, and then generate code to take these locks at the start of the atomic section and release them at the end.
...
```
== Compilers
For the purpose of this project, a compiler consists of a frontend (lexer and parser), a validation phase, and a code-generation phase. Languages typically also include a runtime, which is prewritten code linked with the compiled output to provide environmental support like memory management and scheduling. In _Coherence_, the runtime is critical, acting as a lightweight "operating system" that manages actor mailboxes and actor scheduling.
...
The _Coherence_ runtime is implemented in C++20. This choice was driven by the need for low-level control over system resources. In this section, we will explore the specific features that C++ facilitates. 

==== Fine-grained concurrency
The C++ standard library provide many types for fine-grained concurrency. `std::thread` provides direct access to OS-level threads, `std::mutex` provides access to OS-level locks, and `std::atomic` allows for lock-free access to simple data types. These tools are essential for building the high-performance, thread-safe datastructures that the runtime requires.
...
The above code is a very simple demonstration of how one might implement the runtime scheduler using `fcontext_t`.
- `fcontext_t` is a handle representing the snapshot of the processor's registers before the context switch.
- `make_fcontext` initializes a new `fcontext_t` by taking a pointer to the top of a pre-allocated memory block and a function pointer for the entry point. It sets up the initial state of this stack so that the very first time the runtime switches to it, the CPU begins executing the behaviour's logic as if it were a standard, fresh function call.
...
I chose C++20 as the primary language for developing the _Coherence_ compiler. While the initial decision was driven by my familiarity with the language and its robust support for generating LLVM IR, the project's evolution revealed challenges I had not previously considered. These are sketched out in the (TODO: WILL BE FILLED LATER) chapter. In retrospect, a functional language like OCaml might have been better suited to this task. An unforseen advantage of using C++ was that since the _Coherence_ runtime was written in C++, keeping the compiler in the same language allowed for a unified build system (CMake) and shared testing infrastructure.

== Requirement analysis
The development of _Coherence_ is guided by the following requirements.

- *Static Safety*: Implement a type system and checker that strictly enforces data-race and deadlock freedom.
...
A number of software tools and techniques were used to ensure that the project progressed smoothly. These are described in this section.
=== Build system
I used _CMake_ as the project’s build system. CMake has good integration with VSCode (my IDE) and allowed me to build my project and run tests with a single click.

=== Version control system
I utilized _Git_ for version control. Alot of the implementation choices required significant trial and error. Using Git allowed me to explore new implementation ideas and refactor core logic with the safety net of being able to revert to previous stable states. To safeguard against hardware failure, the repository was hosted remotely on GitHub.
...
- *Initial Core Cycle*: The first iteration focused on implementing a bare-bones version of the language. I implemented a restricted syntax that excluded high-level complexities such as lock-inference for mutually recursive functions or types for multi-level pointers with viewpoint adaptation. The goal of this phase was to establish a functional end-to-end compiler, ensuring that the skeleton of the system was operational. This provided a working foundation early in the project, allowing me to verify the core architecture before committing to the full implementation of more difficult language features.
...
- *Incremental verification*: At the end of each cycle, I wrote tests using the new features to make sure that the features worked as intended.
