...
\section{Pony's expressive power}

Pony safely reintroduces \textit{shared memory} using reference capabilities. This ensures data-race freedom at compile time. However, representing patterns where multiple actors frequently modify a shared data structure remains awkward. Two general patterns predominate:

\subsection{The Manager pattern}

This pattern encapsulates data within a \textit{Manager} actor. All other actors interact with the data by sending asynchronous requests to the Manager.
...
In the code above, threads asynchronously send update or query requests as messages. For querying, the \texttt{IntManager1} is given a sendable callback to which it sends the current value of \texttt{\_data}.

\begin{tcolorbox}[colback=gray!10, colframe=gray!50, sharp corners, boxrule=0pt, leftrule=3pt]
\textbf{Sendable callbacks:} \\
A \textit{sendable} callback is a first-class function that can be dispatched to other actors and executed within their respective execution contexts without the risk of data-races. Since \textit{Coherence} does not implement first-class functions, the specific type-checking rules governing sendable closures will not be discussed. The provided code serves only as a conceptual illustration of the manager pattern.
\end{tcolorbox}
...
\begin{itemize}
    \item Even with lambdas, the code is cluttered with nested closures.
    \item This pattern fails to provide atomicity. For example, between Step 2 and Step 3, a third actor could update \texttt{m1} and \texttt{m2}. The swapper would then overwrite that new value with stale data.
\end{itemize}

\subsection{Hot-Potato Model}

To solve the atomicity problem, developers can use the \textit{hot-potato} model. Here, the manager transfers exclusive ownership of the object to the requester using the \texttt{iso} reference capability. The ownership is returned when the requester is done with the task.
...
This code can be made more robust by ensuring that the \texttt{take} operation never fails. If \texttt{IntManager2} is currently empty (because another actor is holding the potato), it stores the incoming callback in a FIFO queue. When the potato is eventually returned via \texttt{give}, the manager automatically dispatches it to the next callback in the queue.

Assuming this more robust \texttt{IntManager} (call this \texttt{IntManager3}), the code to swap the two shared integers can be written as:
...
\begin{itemize}
    \item The system relies entirely on the requester to return the reference via the \texttt{give} behaviour. If a programmer forgets to return the \textit{potato}, the resource is effectively lost, and all other actors in the queue are blocked indefinitely.
    \item A more fundamental flaw is the indirect introduction of deadlocks. Because ownership is acquired sequentially across multiple actors, the system is susceptible to the \textit{circular wait} condition.
    Consider two actors attempting to access the same two managers simultaneously:
    \begin{itemize}
        \item Actor A calls \texttt{m1.take()} and successfully receives \texttt{p1}. It then calls \texttt{m2.take()} and enters \texttt{m2}'s wait queue.
        \item Actor B calls \texttt{m2.take()} and successfully receives \texttt{p2}. It then calls \texttt{m1.take()} and enters \texttt{m1}'s wait queue.
    \end{itemize}
    Now, Actor A is waiting for Actor B to call \texttt{m2.give()}, and Actor B is waiting for Actor A to call \texttt{m1.give()}. Neither can proceed, and the system is stuck. Pony programs terminate when there are no more messages to run. So in this case, the program simply terminates prematurely.
\end{itemize}

\begin{notebox}
\textbf{Motivation:}\\
The primary goal of \textit{Coherence} is to bridge the gap between the safety of the Actor model and the expressive power of imperative programming.
...
As demonstrated above, there are coordination patterns that are hard to implement without locks. \textit{Coherence} aims to reintroduce locks while maintaining deadlock-freedom. This section describes some concepts needed for this.

\subsection{Locked reference capability}
The standard solution to shared-memory is to protect the shared data structure with a lock, making sure that only one concurrent unit is accessing it at the same time. How can we incorporate this idea in the reference capability model? The Kappa type system by Elias Castegren \cite{kappa_system} does this using the \texttt{locked} reference capability. An object with this capability comes with an implicit lock. Whenever a method of the object is called, the lock associated with the object is taken, which is released after the method execution completes. This object can be shared between actors, and hence allows for easy sharing of mutable state.

We can rewrite the swap logic using the \texttt{locked} capability.
\begin{lstlisting}
class SharedInt
  var _data: U64 = 0
  fun read(): U64 => 
...
\begin{itemize}
    \item Atomicity is still not achieved. As locks are released after every operation, it is still possible for another actor to modify the shared integer values between the read and the write.
    \item In some contrived examples, deadlocks can still occur. This occurs when object fields contain locked references.
\begin{lstlisting}
class Node
...
Consider two \texttt{Node} instances, A and B, where \texttt{A.\_next = B} and \texttt{B.\_next = A}. If two separate actors call \texttt{access\_next()} on A and B simultaneously, the following execution sequence can occur:

\begin{itemize}
    \item Actor 1 acquires the lock for A and enters \texttt{access\_next()}.
    \item Actor 2 acquires the lock for B and enters \texttt{access\_next()}.
    \item Actor 1 attempts to lock B (via \texttt{\_next}) to call \texttt{access()}.
    \item Actor 2 attempts to lock A (via \texttt{\_next}) to call \texttt{access()}.
\end{itemize}
