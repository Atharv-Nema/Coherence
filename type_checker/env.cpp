...
using TypeContext =
    std::unordered_map<std::string, std::shared_ptr<NameableType>>;

// map from function names to their declaration
using FuncNameMap =
    ScopedStore<std::string, std::shared_ptr<TopLevelItem::Func>>;
...
using ActorFrontendMap =
    std::unordered_map<std::string, std::shared_ptr<ActorFrontend>>;
...
class AtomicSectionData {
  ...
  public:
    AtomicSectionData()
        : enclosing_atomic_section(nullptr), enclosing_callable_locks(nullptr) {
    }
    ...
    void add_locks_from_set(
        std::shared_ptr<std::unordered_set<std::string>> lock_set) {
        assert(lock_set != nullptr);
        for (const std::string &s : *lock_set) {
            add_lock(s);
        }
    }
    bool in_atomic_section() { return enclosing_atomic_section != nullptr; }
    ...
    void change_callable(
        std::shared_ptr<std::unordered_set<std::string>> callable_lock_set) {
        enclosing_atomic_section = nullptr;
...
struct TypeEnv {
    ...
    std::shared_ptr<TopLevelItem::Func> curr_func = nullptr;
